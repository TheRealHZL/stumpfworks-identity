.PHONY: build build-server build-agent build-pam-helper build-client-status build-client-diagnostics build-client-updater build-linux-amd64 build-camera-macos run-server run-agent test lint clean
build: build-server build-agent build-pam-helper build-client-status build-client-diagnostics build-client-updater
build-server:
	@mkdir -p bin
...
build-client-updater:
	@mkdir -p bin
	go build -o bin/sw-badge-client-updater ./cmd/client-updater
build-linux-amd64:
	...
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -o dist/linux-amd64/sw-badge-client-updater ./cmd/client-updater
build-agent-linux:
	@mkdir -p dist/linux-amd64
...
lint:
	go vet ./...
clean:
	rm -f bin/sw-badge-server bin/sw-badge-agent bin/sw-badge-pam-helper bin/sw-badge-client-status bin/sw-badge-client-diagnostics bin/sw-badge-client-updater
